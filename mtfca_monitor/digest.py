...
class DigestGenerator:
    ...
    def _build_html(self, digest_type, period_start, period_end, now,
        ...
        if new_topics:
            ...
            for i, t in enumerate(new_topics):
                ...
                topic_rows += f"""<tr style="background:{bg}">
                    ...
                    <td style="padding:8px;font-size:12px;color:#666">{_utc_to_et(t['first_seen'])}</td>
                </tr>"""
            sections.append(f"""
        ...
        if summary.hour_histogram:
            ...
            et_offset = now.utcoffset().total_seconds() / 3600  # e.g. -5 or -4
            et_histogram = {}
            for hour, count in summary.hour_histogram:
                et_hour = int((hour + et_offset) % 24)
                et_histogram[et_hour] = et_histogram.get(et_hour, 0) + count
            sorted_hours = sorted(et_histogram.items())
            ...
            stats_html += f"""
                <h3 style="color:#555;font-size:14px;margin-top:15px">Activity by Hour (ET)</h3>
                {bars}"""
...
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f5f5f5">
    <div style="background:#1a1a2e;color:#eee;padding:25px 30px">
        ...
        <p style="margin:5px 0 0;font-size:13px;color:#aaa">Generated {now.strftime('%Y-%m-%d %I:%M %p ET')} | {digest_type.title()} Digest</p>
    </div>
    ...
    def _build_text(self, digest_type, period_start, period_end, now,
        ...
        lines.append(f"Generated {now.strftime('%Y-%m-%d %I:%M %p ET')} | {digest_type.title()} Digest")
        lines.append("=" * 60)
...
def _utc_to_et(timestamp_str):
    """Convert a UTC timestamp string from the database to ET display format."""
    if not timestamp_str:
        return ""
    try:
        dt = datetime.fromisoformat(timestamp_str).replace(tzinfo=UTC)
        return dt.astimezone(ET).strftime("%Y-%m-%d %I:%M %p")
